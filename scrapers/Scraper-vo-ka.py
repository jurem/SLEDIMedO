...
SOURCE_ID = "VO-KA" # source identifier
NUM_PAGES_TO_CHECK = 3  # how many pages will we check evey day for new articles
MAX_HTTP_RETRIES = 10   # set max number of http request retries if a page load fails

BASE_URL = "http://www.vo-ka.si"
SUB_PAGES_URL = BASE_URL+"/aktualno?page="
...
def getArticleDescr(session, link):
    ...
    return soup.find("div", class_="content--top").text
...
def main():
    ...
    with requests.Session() as s:
        ...
        resp = s.get(SUB_PAGES_URL+"0")
        soup = bs.BeautifulSoup(resp.text, "html.parser")

        # find "next page" button link - to import all the news recursive
        nextPageLink = BASE_URL+soup.find("ul", class_="pager").find("li", class_="pager-next").find("a")["href"] # searches tag "span" with class "next"
        ...
        while nextPageLink != None:
            try:
                ...
                articles = soup.find("div", id="news__blocks").find_all("div", class_="single--block noBg third--width half--height")

                for article in articles:
                    articlesChecked += 1

                    title = article.find("div", class_="title").text             # finds article title
                    link = BASE_URL+article.find("div", class_="title").find("a")["href"] # finds article http link
                    dateStr = article.find("div", class_="date").text
                ...
                # find next page
                try:
                    nextPageLink = BASE_URL+soup.find("ul", class_="pager").find("li", class_="pager-next").find("a")["href"]   # select the "next page" button http link
