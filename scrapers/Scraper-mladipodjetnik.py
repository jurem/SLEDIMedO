...
SOURCE_ID = "MLADIPODJETNIK" # source identifier
NUM_PAGES_TO_CHECK = 1  # how many pages will we check evey day for new articles
MAX_HTTP_RETRIES = 10   # set max number of http request retries if a page load fails

BASE_URL = "https://mladipodjetnik.si"
CATEGORY_URLS = [BASE_URL+cat+"/page/" for cat in ["/novice-in-dogodki/novice",
    "/novice-in-dogodki/aktualni-mp-dogodki", "/novice-in-dogodki/dogodki", "/tag/poslovna-priloznost",
    "/tag/strokovnjak-odgovarja", "/novice-in-dogodki/mp-dogodki"]]
...
def main():
    ...
    with requests.Session() as s:
        ...
        for catNum, categoryLink in enumerate(CATEGORY_URLS):
            logger.info("Checking category {}/{}".format(catNum+1, len(CATEGORY_URLS)))
            resp = s.get(categoryLink+"1")
            soup = bs.BeautifulSoup(resp.text, "html.parser")

            logger.debug("Currently on URL: {}".format(resp.url))
            # find "next page" button link - to import all the news recursive
            nextPageLink = soup.find("div", class_="listingBar").find("span", class_="next").find("a") # searches tag "span" with class "next"
            if nextPageLink:
                nextPageLink = nextPageLink["href"]
            else:
                nextPageLink = "a"  # not None
            ...
            while nextPageLink != None:
                try:
                    ...
                    articles = soup.find("div", id="content").find_all("article", class_="tileItem visualIEFloatFix")
                    logger.debug("Number of found articles: {}".format(len(articles)))

                    for article in articles:
                        articlesChecked += 1

                        title = article.find("h2", class_="tileHeadline").text             # finds article title
                        link = article.find("h2", class_="tileHeadline").find("a")["href"] # finds article http link
                        dateStr = parseDate(article.find("span", class_="documentByLine").text)
