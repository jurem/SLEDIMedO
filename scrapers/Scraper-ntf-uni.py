...
SOURCE_ID = "NTF-UNI" # source identifier
NUM_PAGES_TO_CHECK = 3  # how many pages will we check evey day for new articles
MAX_HTTP_RETRIES = 10   # set max number of http request retries if a page load fails

BASE_URL = "https://www.ntf.uni-lj.si"
SUB_PAGES_URL = BASE_URL+"/aktualno/novice/?cpage="
...
def getArticleDescr(session, link):
    ...
    return soup.find("article").find("div", class_="text-content first-item").text
...
def main():
    ...
    with requests.Session() as s:
        ...
        nextPageLink = soup.find("div", class_="pagination").find("a", class_="next page-numbers")["href"] # searches tag "span" with class "next"
        ...
        while nextPageLink != None:
            try:
                ...
                articles = soup.find_all("div", class_="notification")

                for article in articles:
                    articlesChecked += 1

                    title = article.find("div", class_="title").a.h2.text           # finds article title
                    link = article.find("div", class_="title").find("a")["href"]         # finds article http link
                    dateStr = article.find("div", class_="title").find("small", class_="date-author").text.split(",")[0]
                ...
                # find next page
                try:
                    nextPageLink = soup.find("div", class_="pagination")
                    if nextPageLink is not None:
                        nextPageLink = nextPageLink.find("a", class_="next page-numbers")
                        if nextPageLink is not None:
                            nextPageLink = BASE_URL+nextPageLink["href"]   # select the "next page" button http link
                        else:
                            logger.debug("Probably on the last page.")
                            break
                    else:
                        logger.debug("Probably on the last page.")
                        break
