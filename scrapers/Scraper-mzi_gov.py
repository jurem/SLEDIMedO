...
clanki = []
parent_link = ("http://www.mzi.gov.si/en/media_room/press_release/")
...
def get_text(stran,SOURCE_ID):
    ...
    soup = BeautifulSoup(simple_get("http://www.mzi.gov.si/en/media_room/press_release/page/"+str(stran)), "html.parser")
    all_links = soup.find("div", {"id":"mainContainer2"}).find_all("a")
    for links in all_links:
        if(links.get("href")==None): continue
        if(re.match("http://www.mzi.gov.si/en/media_room/news/+",links.get("href"))):
