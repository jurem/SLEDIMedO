...
SOURCE_ID = "MOJAOBCINA" # source identifier
NUM_PAGES_TO_CHECK = 3  # how many pages will we check evey day for new articles
MAX_HTTP_RETRIES = 10   # set max number of http request retries if a page load fails

BASE_URL = "https://www.mojaobcina.si"
POSTS_URL = BASE_URL+"/litija/novice/iskalnik/?order=date&not-use=ASC&page="
...
def getArticleDescr(session, link):
    ...
    articleDate = parseDate(soup.find("div", class_="single-art-date fl").text)
    subArt = soup.find("div", class_="single-art-sub-title")
    artText = soup.find("div", class_="single-art-text")
    [x.extract() for x in subArt.find_all("div", class_="articleVote")]
    [x.extract() for x in artText.find_all("div", class_="articleVote")]
    return (subArt.text+"\n"+artText.text, articleDate)
...
def main():
    ...
    with requests.Session() as s:
        ...
        nextPageLink = BASE_URL+soup.find("a", class_="paginatorAll paginatorNext")["href"]

        while nextPageLink != None:
            try:
                pagesChecked += 1
                # find all ~15 articles on current page
                articles = soup.find("div", class_="fl newsListingHolder").find_all("div", class_="fl newsTitleContent")

                for article in articles:
                    articlesChecked += 1

                    title = article.find("a", class_="newsTitle").text           # finds article title
                    link = BASE_URL+"/"+article.find("a", class_="newsTitle")["href"]         # finds article http link
                    shortDescr = article.find("a", class_="newsContent").text # finds article date (DATUM_VNOSA)
                    hashStr = makeHash(title, shortDescr)                                      # creates article hash from title and dateStr (HASH_VREDNOST)
                    

                    # if article is not yet saved in the database we add it
                    if sqlBase.getByHash(hashStr) is None:
                        # get article description/content
                        description, dateStr = getArticleDescr(s, link)
                ...
                nextPageLink = soup.find("a", class_="paginatorAll paginatorNext")
                if nextPageLink:
                    nextPageLink = BASE_URL+nextPageLink["href"]   # select the "next page" button http link
