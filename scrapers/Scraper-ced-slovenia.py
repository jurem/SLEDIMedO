...
SOURCE_ID = "CED-SLO" # source identifier
NUM_PAGES_TO_CHECK = 1  # how many pages will we check evey day for new articles
MAX_HTTP_RETRIES = 10   # set max number of http request retries if a page load fails

BASE_URL = "http://ced-slovenia.eu"
CATEGORY_URLS = [BASE_URL+"/kategorija"+category+"/page/" for category in ["/kaj-delamo", "/novosti", "/priloznosti"]]
...
def main():
    ...
    with requests.Session() as s:
        ...
        for catNum, categoryLink in enumerate(CATEGORY_URLS):
            ...
            resp = s.get(categoryLink+"0")
            soup = bs.BeautifulSoup(resp.text, "html.parser")

            # find "next page" button link - to import all the news recursive
            nextPageLink = soup.find("div", class_="nav-links").find("a", class_="next page-numbers")["href"] # searches tag "span" with class "next"
            ...
            while nextPageLink != None:
                try:
                    ...
                    articles = soup.find("div", class_="listing-main").find_all("article")
                    logger.debug("Number of found articles: {}".format(len(articles)))

                    for article in articles:
                        articlesChecked += 1

                        title = article.find("h1", class_="entry-title").text             # finds article title
                        link = article.find("h1", class_="entry-title").find("a")["href"] # finds article http link
                        date_created = article.find("time", class_="entry-date published")["datetime"].split("T")[0]
                    ...
                    # find next page
                    try:
                        nextPageLink = soup.find("div", class_="nav-links").find("a", class_="next page-numbers")["href"]   # select the "next page" button http link
